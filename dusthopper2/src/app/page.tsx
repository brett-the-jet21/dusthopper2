...
export default function Home() {
  ...
  // Launch animation: when countdown hits zero, animate the rocket
  ...
  return (
    <div className="h-screen w-screen bg-black text-white overflow-hidden relative">
      ...
      <div className="absolute inset-0 z-0">
        <MissionScene launchProgress={launchProgress} />
      </div>
      ...
      <header className="relative z-20 p-4 sm:p-6">
        ...
        {data && (
          <p className="text-xs text-white/40 mt-1 ml-6">
            Live data &middot; Updated {new Date(data.updatedAt).toLocaleTimeString()} &middot; Auto-refresh 30s
          </p>
        )}
      </header>
        ...
        {selected && (
          <div className="mb-6 rounded-2xl border border-white/10 bg-black/60 backdrop-blur-xl p-5">
            ...
            <div className="mt-2 text-xs text-white/50">
              {selected.provider} &middot; {selected.vehicle ?? "Vehicle TBD"}
            </div>
            <div className="mt-1 text-xs text-white/50">
              {selected.pad ?? ""}{selected.location ? ` — ${selected.location}` : ""}
            </div>
            ...
            <div className="mt-4 pt-3 border-t border-white/10">
              <Countdown targetDate={selected.net} onLaunch={triggerLaunch} />
            </div>
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <div className="text-sm font-semibold truncate">{launch.name}</div>
            ...
            <div className="space-y-2">
              {recent.map((launch) => (
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <div className="text-sm font-medium truncate text-white/70">{launch.name}</div>
