...
function Sun() {
  return (
    <group position={[50, 0, 0]}>
      ...
      <pointLight color="#ffffff" intensity={100} distance={300} />
      <mesh>
...
function DetailedISS() {
  ...
  return (
    <group ref={groupRef} scale={0.5}>
      <mesh><boxGeometry args={[3, 0.12, 0.12]} /><meshStandardMaterial color="#c0c0c0" metalness={0.9} roughness={0.3} emissive="#c0c0c0" emissiveIntensity={0.4} /></mesh>
      <mesh position={[-0.6, 0, 0]} rotation={[0, 0, Math.PI/2]}><cylinderGeometry args={[0.2, 0.2, 1.2]} /><meshStandardMaterial color="#e8e8e8" metalness={0.7} roughness={0.4} emissive="#e8e8e8" emissiveIntensity={0.3} /></mesh>
      <mesh position={[0.6, 0, 0]} rotation={[0, 0, Math.PI/2]}><cylinderGeometry args={[0.22, 0.22, 1.4]} /><meshStandardMaterial color="#e0e0e0" metalness={0.7} roughness={0.4} emissive="#e0e0e0" emissiveIntensity={0.3} /></mesh>
      <group position={[-1.7, 0, 0]}>
        <mesh position={[0, 0.12, 0]}><boxGeometry args={[2, 0.02, 1]} /><meshStandardMaterial color="#1a3d5c" metalness={0.95} roughness={0.1} emissive="#0066cc" emissiveIntensity={0.4} /></mesh>
        <mesh position={[0, 0.35, 0]}><boxGeometry args={[2, 0.02, 1]} /><meshStandardMaterial color="#1a3d5c" metalness={0.95} roughness={0.1} emissive="#0066cc" emissiveIntensity={0.4} /></mesh>
      </group>
      <group position={[1.7, 0, 0]}>
        <mesh position={[0, 0.12, 0]}><boxGeometry args={[2, 0.02, 1]} /><meshStandardMaterial color="#1a3d5c" metalness={0.95} roughness={0.1} emissive="#0066cc" emissiveIntensity={0.4} /></mesh>
        <mesh position={[0, 0.35, 0]}><boxGeometry args={[2, 0.02, 1]} /><meshStandardMaterial color="#1a3d5c" metalness={0.95} roughness={0.1} emissive="#0066cc" emissiveIntensity={0.4} /></mesh>
      </group>
      <pointLight color="#00ffcc" intensity={4} distance={10} />
    </group>
  );
}
...
function DetailedStarship() {
  ...
  return (
    <group ref={groupRef} scale={0.35}>
      <mesh><cylinderGeometry args={[0.3, 0.3, 2.5, 32]} /><meshStandardMaterial color="#d0d0d0" metalness={0.95} roughness={0.15} emissive="#a0a0a0" emissiveIntensity={0.3} /></mesh>
      <mesh position={[0, 1.6, 0]}><coneGeometry args={[0.3, 0.8, 32]} /><meshStandardMaterial color="#c8c8c8" metalness={0.95} roughness={0.12} emissive="#a0a0a0" emissiveIntensity={0.3} /></mesh>
      ...
      <mesh position={[0.4, 0.6, 0]} rotation={[0, 0, Math.PI/8]}><boxGeometry args={[0.7, 0.05, 0.3]} /><meshStandardMaterial color="#888" metalness={0.9} roughness={0.3} emissive="#666" emissiveIntensity={0.3} /></mesh>
      <mesh position={[-0.4, 0.6, 0]} rotation={[0, 0, -Math.PI/8]}><boxGeometry args={[0.7, 0.05, 0.3]} /><meshStandardMaterial color="#888" metalness={0.9} roughness={0.3} emissive="#666" emissiveIntensity={0.3} /></mesh>
      <pointLight color="#ffaa00" intensity={4} distance={10} />
    </group>
  );
}
...
function DetailedStarlink() {
  return (
    <group scale={0.3}>
      <mesh><boxGeometry args={[0.8, 0.1, 0.5]} /><meshStandardMaterial color="#2a2a2a" metalness={0.9} roughness={0.2} emissive="#2a2a2a" emissiveIntensity={0.4} /></mesh>
      <mesh position={[0, 0.15, 0]}><boxGeometry args={[1.2, 0.02, 0.4]} /><meshStandardMaterial color="#1a4d7a" metalness={0.95} roughness={0.1} emissive="#0066cc" emissiveIntensity={0.5} /></mesh>
      <pointLight color="#00ff88" intensity={4} distance={8} />
    </group>
  );
}

// FIXED: Spacecraft that generates its own orbit path
...
function RotatingEarth({ timeScale }: { timeScale: number }) {
  ...
  useEffect(() => {
    // Set initial rotation based on current UTC time
    const now = new Date();
    const utcHours = now.getUTCHours() + now.getUTCMinutes() / 60 + now.getUTCSeconds() / 3600;
    rotationRef.current = (utcHours / 24) * Math.PI * 2;
  }, []);
  
  useFrame((state, delta) => {
    if (earthRef.current) {
      if (timeScale > 0) {
        // 86400 seconds per full rotation
        const rotationSpeed = (Math.PI * 2) / 86400;
        rotationRef.current += rotationSpeed * delta * timeScale;
      }
...
function SpaceCamera({ target, enabled, zoom }: any) {
  ...
  useFrame(() => {
    if (enabled && target && target.length === 3) {
      ...
      const distance = 15 / zoom; // FIXED zoom calculation
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#000' }}>
      <Canvas camera={{ position: [0, 12, 22], fov: 50 }}>
        ...
        <directionalLight position={[50, 0, 0]} intensity={10} castShadow />
        <StarsField />
        ...
        {playing && missions.map((m, i) => (
          <OrbitingSpacecraft key={i} mission={m} index={i} timeScale={timeScale} onPositionUpdate={updatePosition(i)} onPathUpdate={updatePath} />
        ))}
      ...
      <div style={{position:'fixed',top:90,right:25,width:280,background:'rgba(0,15,30,0.98)',border:'2px solid rgba(0,200,255,0.4)',borderRadius:10,padding:'20px 24px',color:'#0cf',fontSize:12,fontFamily:'monospace',boxShadow:'0 8px 32px rgba(0,0,0,0.9)',zIndex:100}}>
        <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:18,paddingBottom:16,borderBottom:'2px solid rgba(0,200,255,0.3)'}}>
          <div style={{width:10,height:10,borderRadius:'50%',background:'#0f8',boxShadow:'0 0 15px #0f8',animation:'pulse 2s infinite'}}/>
