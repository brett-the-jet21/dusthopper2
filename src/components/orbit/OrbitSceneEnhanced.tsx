...
function AnimatedSpacecraft({ mission, index, timeScale, onPositionUpdate }: any) {
  const groupRef = useRef<THREE.Group>(null);
  
  useFrame((state) => {
    if (groupRef.current) {
      const orbitalPeriod = 90 * 60; // 90 minutes
      const scaledTime = state.clock.elapsedTime * timeScale;
      const progress = (scaledTime / orbitalPeriod) % 1;
...
function RotatingEarth({ timeScale }: { timeScale: number }) {
  ...
  useFrame((state) => {
    if (earthRef.current) {
      // Earth rotates once every 24 hours in real life
      const dayDuration = 24 * 60 * 60; // seconds
      const scaledTime = state.clock.elapsedTime * timeScale;
      const rotationAngle = (scaledTime / dayDuration) * Math.PI * 2;
...
export function OrbitSceneEnhanced({ missionId }: { missionId: string }) {
  ...
  const [timeScale, setTimeScale] = useState(100); // Start at 100× speed
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#000' }}>
      ...
      <div style={{position:'fixed',top:20,left:'50%',transform:'translateX(-50%)',display:'flex',gap:12,background:'rgba(5,10,15,0.95)',padding:'12px 24px',borderRadius:10,border:'1px solid rgba(0,200,255,0.25)',zIndex:100}}>
        ...
        <div style={{width:1,height:30,background:'rgba(0,200,255,0.2)'}}/>
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          <span style={{color:'rgba(0,200,255,0.7)',fontSize:11,fontWeight:600}}>TIME</span>
          <select value={timeScale} onChange={(e)=>setTimeScale(Number(e.target.value))} style={{background:'#1a1a1a',color:'#0cf',border:'1px solid #0cf',padding:'8px 14px',borderRadius:6,cursor:'pointer',fontWeight:700,fontSize:11,fontFamily:'monospace'}}>
      ...
      {/* MISSION SELECT */}
      <div style={{position:'fixed',bottom:20,left:'50%',transform:'translateX(-50%)',display:'flex',gap:12,zIndex:100}}>{missions.map((m,i)=><button key={i} onClick={()=>setSelectedMission(i)} style={{background:i===selectedMission?m.color:'rgba(5,10,15,0.95)',color:i===selectedMission?'#000':'#fff',border:`2px solid ${m.color}`,padding:'10px 20px',borderRadius:8,cursor:'pointer',fontWeight:700,fontSize:11,boxShadow:i===selectedMission?`0 0 20px ${m.color}80`:'none'}}>{m.name}</button>)}</div>
      
      {/* TELEMETRY */}
      <div style={{position:'fixed',top:20,right:20,width:240,background:'rgba(5,10,15,0.95)',border:'1px solid rgba(0,200,255,0.25)',borderRadius:10,padding:16,color:'#0cf',fontSize:11,fontFamily:'monospace',zIndex:100}}>
        <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12,paddingBottom:10,borderBottom:'1px solid rgba(0,200,255,0.15)'}}>
          <div style={{width:6,height:6,borderRadius:'50%',background:'#0f8',boxShadow:'0 0 8px #0f8'}}/>
          <div style={{fontWeight:700,fontSize:12}}>{missions[selectedMission].name}</div>
        </div>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}>
          <span style={{opacity:0.6}}>ALTITUDE</span>
          <span style={{fontWeight:600}}>{missions[selectedMission].alt} km</span>
        </div>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}>
          <span style={{opacity:0.6}}>VELOCITY</span>
          <span style={{fontWeight:600}}>7.66 km/s</span>
        </div>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}>
          <span style={{opacity:0.6}}>INCLINATION</span>
          <span style={{fontWeight:600}}>{missions[selectedMission].inclination}°</span>
        </div>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:10}}>
          <span style={{opacity:0.6}}>PERIOD</span>
          <span style={{fontWeight:600}}>~90 min</span>
        </div>
        <div style={{borderTop:'1px solid rgba(0,200,255,0.15)',paddingTop:10}}>
          <div style={{opacity:0.5,fontSize:10}}>TIME SCALE</div>
          <div style={{fontWeight:700,fontSize:14,color:'#0ff'}}>{timeScale}× {timeScale===1&&'REAL-TIME'}</div>
        </div>
      </div>
    </div>
  );
